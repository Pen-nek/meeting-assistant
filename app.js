...
const state = {
    ...
    utterances: [], speakerNames: {}, topics: [],
    ...
    ctxTarget: null, cpDrag: false,
    // 현재 재생 중인 utterance-item
    activeItem: null,
};
...
const dlBlob  = (blob, name) => Object.assign(document.createElement('a'),
    { href: URL.createObjectURL(blob), download: name }).click();
...
function syncTodosFromDOM() {
    ...
    const newOrder = [];
    $$('#todoContent .todo-item').forEach(li => {
        const task = li.querySelector('.todo-task')?.innerText.trim();
        const found = state.result.todos.find(t => t.task === task);
        if (found) newOrder.push(found);
    });
    state.result.todos = newOrder;
}

// ── 탭별 맨 위로 스크롤 ──────────────────────────
function scrollTabTop() {
    // 각 탭의 tab-section-title로 스크롤
    const tabEl = $('tab-' + state.tab);
    const title = tabEl && tabEl.querySelector('.tab-section-title');
...
function setupPlayer() {
    ...
    const seek = x => {
        ...
        audioEl.currentTime = Math.max(0, Math.min(1, (x - r.left) / r.width)) * (audioEl.duration || 0);
    };
    ...
    $('cpVolBtn').addEventListener('click', () => {
        ...
        $('cpVolSlider').value = audioEl.muted ? 0 : audioEl.volume;
        syncVolIcon();
    });
    $('cpVolSlider').addEventListener('input', e => {
...
function setupRateMenu(btnId, menuId) {
    ...
    btn.addEventListener('click', e => {
        ...
        if (opening) menu.classList.toggle('drop-down', btn.getBoundingClientRect().top < 180);
    });
...
function syncVolIcon() {
    const w1 = $('cpVolWave1'), w2 = $('cpVolWave2');
    if (!w1) return;
...
// 현재 재생 중인 play-btn 하이라이트 + utterance-item 하이라이트
...
function onTick() {
    ...
    if (state.stickyVisible) syncStickyProgress(pct, cur);
    ...
    if (hit !== state.activeItem) {
        ...
        if (state.activeItem) {
            ...
            const prevBtn = state.activeItem.querySelector('.play-btn');
        ...
        if (hit && !audioEl.paused) {
            ...
            const btn = hit.querySelector('.play-btn');
...
function createStickyBar() {
      ...
      <div class="sticky-controls">
        <div class="cp-vol-wrap">
          <button class="cp-vol-btn sticky-vol-btn" id="stickyVolBtn" title="음소거">
            <svg viewBox="0 0 16 16"><path d="M2 5h3l4-3v12l-4-3H2V5z"/><path id="stickyVolWave1" d="M11 4.5a5 5 0 0 1 0 7"/><path id="stickyVolWave2" d="M12.5 2.5a8 8 0 0 1 0 11"/></svg>
          </button>
        ...
        <div class="cp-rate-wrap">
          ...
          <div class="cp-rate-menu sticky-rate-menu" id="stickyRateMenu">
            ...
            <button data-rate="1" class="${audioEl.playbackRate===1?'active':''}">1×</button>
    ...
    const ss = x => {
        ...
        const t = Math.max(0, Math.min(1, (x - r.left) / r.width)) * (audioEl.duration || 0);
        audioEl.currentTime = t; seekScrollToTime(t);
    };
    ...
    // 볼륨 (sticky → 메인과 동기)
    $('stickyVolBtn').addEventListener('click', () => {
        audioEl.muted = !audioEl.muted;
        $('stickyVolSlider').value = audioEl.muted ? 0 : audioEl.volume;
        syncStickyVolIcon();
        syncVolIcon();
    });
    $('stickyVolSlider').addEventListener('input', e => {
        audioEl.volume = +e.target.value; audioEl.muted = audioEl.volume === 0;
        $('cpVolSlider').value = e.target.value;
        syncStickyVolIcon(); syncVolIcon();
    });
...
function syncStickyProgress(pct, cur) {
...
function syncStickyVolIcon() {
    const w1 = $('stickyVolWave1'), w2 = $('stickyVolWave2');
    if (!w1) return;
    const v = audioEl.muted ? 0 : audioEl.volume;
    w1.style.display = v > 0   ? '' : 'none';
    w2.style.display = v > 0.5 ? '' : 'none';
...
function trackScroll(cur) {
    ...
    state.lastScrollEl = target;
    state.scrollPaused = true;

    // 화면이 넓을 때(태블릿 이상) 이전 아이템부터 보이도록 스크롤
    const scrollTarget = getScrollAnchor(target);
    scrollTarget.scrollIntoView({ behavior: 'smooth', block: 'start' });
    setTimeout(() => { state.scrollPaused = false; }, 800);
}

/**
 * 현재 utterance-item 기준으로 스크롤 앵커를 결정.
 * - 같은 speaker-group 내 이전 utterance-item이 있으면 그것
 * - 그룹의 첫 번째 아이템이면 speaker-group 헤더(speaker-group-header)
 * - 완전히 첫 아이템이면 자기 자신
 */
function getScrollAnchor(el) {
    if (window.innerWidth < 768) return el; // 모바일은 그대로

    const prevSibling = el.previousElementSibling;
    if (prevSibling && prevSibling.classList.contains('utterance-item')) {
        return prevSibling;
    }
    // 첫 utterance-item — 부모 speaker-group의 헤더로
    const group = el.closest('.speaker-group');
    if (group) {
        const header = group.querySelector('.speaker-group-header');
...
function getUtteranceAtTime(cur) {
    const items = $$('.utterance-item');
    if (!items.length) return null;
    let lo = 0, hi = items.length - 1, result = null;
    while (lo <= hi) {
        const mid = (lo + hi) >> 1;
        if (parseFloat(items[mid].dataset.start) <= cur) { result = items[mid]; lo = mid + 1; }
        else hi = mid - 1;
    }
    return result;
}
...
window.addEventListener('scroll', () => {
    ...
    const audioRect = $('audioPlayerResult').getBoundingClientRect();
    if (audioRect.bottom < 0 && !state.stickyVisible) {
        createStickyBar(); state.stickyBar.classList.add('visible'); state.stickyVisible = true;
        syncStickyProgress((audioEl.currentTime/(audioEl.duration||1))*100, audioEl.currentTime);
    } else if (audioRect.bottom >= 0 && state.stickyVisible) {
...
function buildSpeakerPanel(utterances) {
    ...
    speakers.forEach(spk => {
        ...
        card.innerHTML = `
      <div class="speaker-card-dot" style="background:${c.dot}"></div>
      <input class="speaker-card-name" value="${spkName(spk)}" data-spk="${spk}">
      <button class="speaker-card-play" data-spk="${spk}" title="미리듣기">▶</button>`;
...
function renderTranscript(utterances) {
    ...
    const n=Math.min(VIRTUAL_BATCH,utterances.length);
...
function appendMore() {
    ...
    const next=Math.min(state.rendered+VIRTUAL_BATCH,state.utterances.length);
...
function rebuildFromDOM() {
    renderTranscript($$('.utterance-item').map(el=>({
        speaker:+el.querySelector('.utterance-text').dataset.speaker-1,
        text:el.querySelector('.utterance-text').innerText.trim(),
...
function makeMinutesRow(line) {
    ...
    handle.className = 'drag-handle'; handle.textContent = '⠿'; handle.title = '드래그해서 이동';
    ...
    del.className = 'row-del-btn'; del.textContent = '✕'; del.title = '삭제';
    del.addEventListener('click', () => {
        if (!confirm('이 항목을 삭제할까요?')) return;
        wrap.remove(); syncMinutes();
    });

    wrap.appendChild(handle); wrap.appendChild(el); wrap.appendChild(del);
    addDragEvents(wrap);
    return wrap;
}
...
$('addMinutesBtn').addEventListener('click', () => {
    const c = $('minutesContent');
    // 이미 폼이 있으면 제거
    c.querySelector('.minutes-add-form')?.remove();
    const form = document.createElement('div'); form.className = 'minutes-add-form';
    form.innerHTML = `
    <select class="custom-sel minutes-type-sel">
      <option value="p">일반 항목</option>
      <option value="h">헤더 항목</option>
    </select>
    <input class="todo-add-input" placeholder="내용 입력..." />
    ...
    form.querySelector('.todo-add-confirm').addEventListener('click', () => {
        ...
        const isH = form.querySelector('select').value === 'h';
        const line = isH ? `## ${text}` : text;
        const row = makeMinutesRow(line);
...
function makeTodo(t, todosArr) {
    ...
    li.className = 'todo-item'; li.draggable = true;

    const pc = {'높음':'priority-high','낮음':'priority-low'}[t.priority] ?? 'priority-mid';
    const speakers = [...new Set($$('.utterance-text').map(el=>+el.dataset.speaker).filter(n=>!isNaN(n)))].sort((a,b)=>a-b);

    // 담당자 현재 값 레이블
    const curOwnerLabel = (t.speaker !== null && t.speaker !== undefined)
        ? spkName(t.speaker + 1) : '미정';
    ...
    <div class="todo-chips">
      <div class="td-dropdown prio-dd ${pc}" tabindex="0">
        ...
        <div class="td-menu prio-menu">
          <div class="td-opt ${curPrio==='높음'?'active':''} priority-high" data-val="높음">높음</div>
          <div class="td-opt ${curPrio==='보통'?'active':''} priority-mid" data-val="보통">보통</div>
          <div class="td-opt ${curPrio==='낮음'?'active':''} priority-low" data-val="낮음">낮음</div>
        </div>
      </div>
...
(function initConfigControls() {
    ...
    const update = n => { inp.value = n; val.textContent = n; };
    $('stepperDown').addEventListener('click', () => update(Math.max(2, +inp.value - 1)));
    $('stepperUp').addEventListener('click',   () => update(Math.min(10, +inp.value + 1)));
